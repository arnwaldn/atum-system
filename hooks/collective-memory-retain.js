...
function main() {
  ...
  var stdinRaw = readStdin();
  var stopReason = "";
  try { stopReason = JSON.parse(stdinRaw).stop_reason || ""; } catch { /* ok */ }
  ...
  // Write LOCAL session summary (replaces session-memory.js)
  writeLocalSessionSummary(stats || {}, dateStr, timeStr, duration, shortId, shortProject, stopReason);

  // Write collective session file
...
/**
 * Write local session summary (absorbed from session-memory.js).
 * Saves to: ~/.claude/projects/<project>/memory/sessions/YYYY-MM-DD-<id>.md
 * Auto-cleanup: files older than 3 days.
 */
function writeLocalSessionSummary(stats, dateStr, timeStr, duration, shortId, shortProject, stopReason) {
  try {
    var projectDir = process.env.CLAUDE_PROJECT_DIR || "";
    var home = process.env.HOME || process.env.USERPROFILE || "";
    var memDir;
    if (projectDir) {
      var escaped = projectDir.replace(/[\\/:]/g, "-").replace(/^-+|-+$/g, "");
      memDir = path.join(home, ".claude", "projects", escaped, "memory", "sessions");
    } else {
      memDir = path.join(home, ".claude", "memory", "sessions");
    }
    fs.mkdirSync(memDir, { recursive: true });

    var totalCalls = stats.totalCalls || 0;
    if (totalCalls < 8) return; // skip trivial sessions

    var lines = [
      "# Session: " + dateStr + " " + timeStr,
      "**ID**: " + shortId + " | **Duration**: " + duration + " | **Calls**: " + totalCalls + " | **Errors**: " + (stats.errors || 0),
      "",
    ];
    if (stats.toolCounts) {
      lines.push("## Tools");
    ...
    if (stats.filesModified && stats.filesModified.length > 0) {
      ...
      stats.filesModified.forEach(function(f) { lines.push("- " + shortenPath(f)); });
      lines.push("");
    }
    if (stats.filesRead && stats.filesRead.length > 0) {
      lines.push("## Files Read");
      var shown = stats.filesRead.slice(0, 10);
      shown.forEach(function(f) { lines.push("- " + shortenPath(f)); });
      if (stats.filesRead.length > 10) lines.push("- ...and " + (stats.filesRead.length - 10) + " more");
      lines.push("");
    }
    lines.push("## Context");
    lines.push("- **Project**: " + (shortProject || projectDir ? shortenPath(projectDir) : "global"));
    lines.push("- **Stop**: " + (stopReason || "user"));
    lines.push("");
    lines.push("---");
    lines.push("*Auto-generated by collective-memory-retain (absorbed session-memory)*");
    lines.push("");

    var fn = dateStr + "-" + shortId + ".md";
    var fp = path.join(memDir, fn);
    if (fs.existsSync(fp)) {
      var c = 2;
      while (fs.existsSync(path.join(memDir, dateStr + "-" + shortId + "-" + c + ".md"))) c++;
      fp = path.join(memDir, dateStr + "-" + shortId + "-" + c + ".md");
    }
    fs.writeFileSync(fp, lines.join("\n"));

    // Clean old files (3 days)
    var cutoff = Date.now() - 3 * 86400000;
    fs.readdirSync(memDir).filter(function(f) { return /^\d{4}-\d{2}-\d{2}-.+\.md$/.test(f); }).forEach(function(f) {
      var st = fs.statSync(path.join(memDir, f));
      if (st.mtimeMs < cutoff) fs.unlinkSync(path.join(memDir, f));
    });
