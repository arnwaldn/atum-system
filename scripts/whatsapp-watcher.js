...
function flushBuffer() {
  ...
  spawnClaude(fullPrompt, tmpFile, 'opus');
}

// --- Spawn Claude CLI with model fallback ---
function spawnClaude(prompt, tmpFile, model) {
  log(`Spawning Claude ${model}...`);
  ...
  claude.on('close', (code) => {
    // Cleanup temp file only on final success or non-retryable error
    const output = stdout.trim();
    const isApiRefusal = code !== 0 && output.includes('Usage Policy');

    log(`Claude ${model} exited (code ${code})`);
    if (output) log(`Output: ${output.slice(0, 500)}`);
    if (stderr.trim()) log(`Errors: ${stderr.trim().slice(0, 300)}`);

    // Fallback: if opus refused (API policy), retry with sonnet
    if (isApiRefusal && model === 'opus') {
      log('Opus refused (API policy) — retrying with sonnet...');
      spawnClaude(prompt, tmpFile, 'sonnet');
      return;
    }
