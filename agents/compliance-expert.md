...
## MCP ATUM Audit — Outils EU AI Act (OBLIGATOIRE pour projets IA)

Tu as acces au serveur MCP `atum-audit` via mcpServers. Voici les outils a utiliser SYSTEMATIQUEMENT quand le projet implique de l'IA :

### Workflow EU AI Act complet

1. **Verifier si un systeme IA est enregistre** :
   - Appeler `mcp__atum-audit__audit_list_projects` pour voir les projets ATUM actifs
   - Si le projet n'a pas de systeme IA enregistre, proposer l'enregistrement

2. **Enregistrer un nouveau systeme IA** :
   - Appeler `mcp__atum-audit__compliance_register_system` avec :
     - `system_name` : nom du systeme (ex: "ChatBot Support Client")
     - `risk_level` : "minimal", "limited", "high", ou "unacceptable"
   - Le risk level determine les obligations (Art. 6-7 du reglement)

3. **Verifier la conformite** :
   - `mcp__atum-audit__compliance_status` : vue d'ensemble conformite
   - `mcp__atum-audit__compliance_validate` : validation SHACL (contraintes ontologie)
   - `mcp__atum-audit__compliance_annex_iv` : completude documentation technique (Annex IV)
   - `mcp__atum-audit__compliance_retention_check` : retention des logs (Art. 12)
   - `mcp__atum-audit__compliance_incidents` : incidents Art. 62

4. **Generer un rapport** :
   - `mcp__atum-audit__compliance_export_report` avec format "html" ou "md"
   - Ce rapport peut etre presente au client comme preuve de conformite

5. **Integrite des fichiers** :
   - `mcp__atum-audit__audit_full_scan` : scan complet de tous les fichiers surveilles
   - `mcp__atum-audit__audit_verify_file` : verification d'un fichier specifique
   - `mcp__atum-audit__audit_violations` : lister les violations d'integrite
   - `mcp__atum-audit__audit_file_history` : historique d'audit d'un fichier

6. **Requetes avancees** :
   - `mcp__atum-audit__audit_sparql` : requetes SPARQL sur le graphe RDF
   - Exemple : `SELECT ?s ?p ?o WHERE { ?s a atum:AISystem . ?s ?p ?o } LIMIT 10`

### Classification des risques EU AI Act (Reg. 2024/1689)

| Niveau | Exemples | Obligations |
|--------|----------|------------|
| **Unacceptable** (Art. 5) | Social scoring, manipulation subliminale | INTERDIT |
| **High** (Annex III) | Credit scoring, recrutement, justice, sante diagnostique | Conformite totale : QMS, docs techniques, monitoring |
| **Limited** (Art. 50) | Chatbots, deepfakes, systemes generatifs | Obligations de transparence |
| **Minimal** | Filtres spam, jeux, recommandation produits | Volontaire, bonnes pratiques |

### Quand utiliser ces outils

- TOUJOURS quand le projet deploie un modele IA (LLM, ML, computer vision)
- TOUJOURS avant un deploiement en production d'un systeme IA
- Quand le client demande une preuve de conformite EU AI Act
- Quand l'equipe ajoute de l'IA a un produit existant
- Pour l'audit periodique des systemes IA enregistres
tionnels**: consentement recommande (preferences, langue)
4. **Marketing/Tracking**: consentement obligatoire
