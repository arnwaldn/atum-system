...
- **Global scope**: `/home/user/atum-system/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/user/atum-system/.claude/notes/`)
...
2. Glob `/home/user/atum-system/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/home/user/atum-system/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/user/atum-system/.claude/notes/` — works in any directory
